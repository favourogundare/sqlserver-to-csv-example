...
## 1. initialize the target csv with specific name and set header using the columns
my_csv = TargetCSV('only_pdf_target.csv').set_head([
    'LEGACY_INCIDENT_ID',
    'FILENAME',
    'FILETYPE'
])
...
for row in source_db.iterate_table('FILE_MAPPING_SOURCE'):
    ...
    manipulated_row['LEGACY_INCIDENT_ID'] = row['LEGACY_INCIDENT_ID']
    manipulated_row['FILENAME'] = row['FILENAME']
    if get_filetype_map(row['FILETYPE']) == 'application/pdf':
        manipulated_row['FILETYPE'] = get_filetype_map(row['FILETYPE'])
        ## add the row to the csv only if filetype is pdf
        my_csv.append_row(manipulated_row)  
...
## Scenerio 5
'''
1. iterate over table `FILE_MAPPING_SOURCE` and get the `LEGACY_INCIDENT_ID` and `FILENAME` and `ENTER_DATE`
2. enter date should be in format YYYY-MM-DD
3. save into target csv `FILE_MAPPING_TARGET` with columns `LEGACY_INCIDENT_ID` and `FILENAME` and `ENTER_DATE`
'''
...
## 1. initialize the target csv with specific name and set header using the columns
my_csv = TargetCSV('date_manipulated_target.csv').set_head([
    'LEGACY_INCIDENT_ID',
    'FILENAME',
    'ENTER_DATE'
])
...
for row in source_db.iterate_table('FILE_MAPPING_SOURCE'):
    ...
    manipulated_row['FILENAME'] = row['FILENAME']
    if row['ENTER_DATE']:
        manipulated_row['ENTER_DATE'] = row['ENTER_DATE'].strftime('%Y-%m-%d')
